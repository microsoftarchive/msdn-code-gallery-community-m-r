...
Public Class SqlTileServerAzure
    ...
    Public myAzureStorageUri As String = "http://donkeystorage.blob.core.windows.net/tiles/SqlTrails"
    Public myBlobUri As String = ""
    Public isCached As Boolean = False
    Public myAzureAccountName As String = "YOUR_AZURE_STORAGE_ACCOUNT_NAME"
    Public myAzureAccountKey As String = "YOUR_AZURE_STORAGE_KEY"
    Public myAzureContainerRef As String = "YOUR_AZURE_CONTAINER_REF"
    ...
    Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        'Is file in cached?
        myBlobUri = myAzureStorageUri + "/" + quadkey + ".png"
        Try
            ...
            Dim myImage As New Bitmap(System.Drawing.Image.FromStream(response.GetResponseStream))
            isCached = True
        ...
        Catch ex As Exception
            ...
            Dim settings As ConnectionStringSettings = ConfigurationManager.ConnectionStrings("azure")
                ...
                For loop1 = 1 To numGeom
                    ...
                    If curGeom.STNumPoints > 2 Then
                        ...
                        For loop2 = 1 To numPoints
                            ...
                            PointArray(loop2 - 1) = New Point(myPixelX - nwX, myPixelY - nwY)
                        Next
    ...
    Private Sub WritePngToStream(ByVal image As Bitmap, ByVal outStream As Stream)
        ...
        If isCached = False Then
            ...
            Dim MyBlob = Me.GetContainer().GetBlockBlobReference(quadkey + ".png")
    ...
    Private Sub EnsureContainerExists()
        ...
        permissions.PublicAccess = Blob.BlobContainerPublicAccessType.Container
        container.SetPermissions(permissions)
    End Sub

    Private Function GetContainer() As Blob.CloudBlobContainer
        ' Get a handle on account, create a blob storage client and get container proxy
        Dim account = CloudStorageAccount.Parse("DefaultEndpointsProtocol=http;AccountName=" + myAzureAccountName + ";AccountKey=" + myAzureAccountKey)
        Dim client = account.CreateCloudBlobClient()
        Return client.GetContainerReference(myAzureContainerRef)
    End Function
