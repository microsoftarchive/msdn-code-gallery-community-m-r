...
Public Class frmSEARCH_EXPENSE_DETAIL
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmSEARCH_EXPENSE_DETAIL))
        ...
        Me.GroupBox2.Controls.Add(Me.TxtExpense)
        ...
        Me.TxtExpense.Font = New System.Drawing.Font("Verdana", 8.25!, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
        Me.TxtExpense.Location = New System.Drawing.Point(66, 16)
        Me.TxtExpense.MaxLength = 50
        Me.TxtExpense.Name = "TxtExpense"
        Me.TxtExpense.Size = New System.Drawing.Size(164, 21)
        ...
        Me.Label3.Text = "Expenses Detail"
        ...
        'daEXPENSE_DETAIL
        '
        Me.daEXPENSE_DETAIL.SelectCommand = Me.SqlSelectCommand1
        ...
        Me.Name = "frmSEARCH_EXPENSE_DETAIL"
        Me.ShowInTaskbar = False
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "EXPENSE DETAIL HISTORY"
...
#Region "FORM CONTROL"
    Private Sub frmSEARCH_EXPENSE_DETAIL_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.SqlConnection1.ConnectionString = Me.asConn.Conn.ConnectionString
        Me.FillListView()
    End Sub

    Private Sub frmSEARCH_EXPENSE_DETAIL_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        Me.asNum.EnterTab(e)
    End Sub
#End Region

#Region "TextBox Control"
    Private Sub Txt_GotFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtExpense.GotFocus, TxtDateFrom.GotFocus, TxtDateTo.GotFocus
        CType(sender, TextBox).BackColor = Color.LightSteelBlue
        CType(sender, TextBox).SelectAll()
    End Sub
    Private Sub Txt_LostFocus(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtExpense.LostFocus, TxtDateFrom.LostFocus, TxtDateTo.LostFocus
...
#Region "ListView Control"
    Private Sub ListView1_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListView1.DoubleClick
        ...
        If Not Me.ListView1.SelectedItems(0).Text = Nothing Then
            My.Forms.frmEXPENSE_DETAIL.LblID.Text = Me.ListView1.SelectedItems(0).Text

            My.Forms.frmEXPENSE_DETAIL.BttnSave.Enabled = True
            Me.Close()
...
#Region "Button Control"
    Private Sub BttnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BttnSearch.Click
        ...
        If Not Me.TxtExpense.Text = Nothing And Me.TxtDateFrom.Text = Nothing And Me.TxtDateTo.Text = Nothing Then
            Str2 = "SELECT ID, dDATE, EXP_NAME, EXP_DESC, CONVERT(NUMERIC(18, 2), CASH_AMT) AS CASH_AMT, CHQ_NO, CHQ_TYPE, CHQ_DATE, CONVERT(NUMERIC(18, 2), BANK_AMT) AS BANK_AMT, BANK_ACC, GROUP_NAME, USER_NAME, GROUP_ID FROM V_EXPENSES WHERE EXP_NAME LIKE '%" & Me.TxtExpense.Text & "%' ORDER BY ID DESC"

            Me.FillListView_Condition()

        ElseIf Not Me.TxtDateFrom.Text = Nothing And Not Me.TxtDateTo.Text = Nothing And Me.TxtExpense.Text = Nothing Then
            Str2 = "SELECT ID, dDATE, EXP_NAME, EXP_DESC, CONVERT(NUMERIC(18, 2), CASH_AMT) AS CASH_AMT, CHQ_NO, CHQ_TYPE, CHQ_DATE, CONVERT(NUMERIC(18, 2), BANK_AMT) AS BANK_AMT, BANK_ACC, GROUP_NAME, USER_NAME, GROUP_ID FROM V_EXPENSES WHERE dDATE >= CONVERT(DATETIME, '" & Me.TxtDateFrom.Text & "') AND dDATE <= CONVERT(DATETIME, '" & Me.TxtDateTo.Text & "') ORDER BY ID DESC"
            Me.FillListView_Condition()

        ElseIf Not Me.TxtExpense.Text = Nothing And Not Me.TxtDateFrom.Text = Nothing And Not Me.TxtDateTo.Text = Nothing Then
            Str2 = "SELECT ID, dDATE, EXP_NAME, EXP_DESC, CONVERT(NUMERIC(18, 2), CASH_AMT) AS CASH_AMT, CHQ_NO, CHQ_TYPE, CHQ_DATE, CONVERT(NUMERIC(18, 2), BANK_AMT) AS BANK_AMT, BANK_ACC, GROUP_NAME, USER_NAME, GROUP_ID FROM V_EXPENSES WHERE EXP_NAME LIKE '%" & Me.TxtExpense.Text & "%' AND dDATE >= CONVERT(DATETIME, '" & Me.TxtDateFrom.Text & "') AND dDATE <= CONVERT(DATETIME, '" & Me.TxtDateTo.Text & "') ORDER BY ID DESC"
            Me.FillListView_Condition()
...
#Region "Sub and Functions"
    Private Sub FillListView()
        Try
            Dim Str1 As String = "SELECT ID, dDATE, EXP_NAME, EXP_DESC, CONVERT(NUMERIC(18, 2), CASH_AMT) AS CASH_AMT, CHQ_NO, CHQ_TYPE, CHQ_DATE, CONVERT(NUMERIC(18, 2), BANK_AMT) AS BANK_AMT, BANK_ACC, GROUP_NAME, USER_NAME, GROUP_ID FROM V_EXPENSES ORDER BY ID DESC"
            ...
            For Cnt = 0 To Me.DsEXPENSE_DETAIL1.V_EXPENSES.Count - 1
                LstItem = Me.ListView1.Items.Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(0).ToString)
                Me.ListView1.Items(Cnt).UseItemStyleForSubItems = False
                With LstItem.SubItems

                    .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(2).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    Dim dDATE As String = Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(1).ToString

                    'MsgBox(Me.DsSUPPLIER_PAYMENT1.V_SUPPLIER_PAYMENT.Item(Cnt).Item(3).ToString)

                    If dDATE.Length > 0 Then
                        .Add(CDate(dDATE).ToString("dd-MMM-yyyy"), Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    Else
                        .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(1).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    End If

                    .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(4).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(8).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(9).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
                    .Add(Me.DsEXPENSE_DETAIL1.V_EXPENSES.Item(Cnt).Item(3).ToString, Color.DarkBlue, Me.ListView1.BackColor, Me.ListView1.Font)
    ...
    Private Sub FillListView_Condition()
        Try
            ...
            Me.daEXPENSE_DETAIL = New SDS.SqlDataAdapter(SqlCmd2)
